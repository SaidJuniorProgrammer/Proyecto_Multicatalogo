...
export const contarRed = (raiz: Referido): number =>
  (raiz.hijos ?? []).reduce((total, hijo) => total + 1 + contarRed(hijo), 0);
...
export const comisionDeReferido = (referido: Referido): number =>
  referido.ventas * (TASA_COMISION[referido.nivel] ?? 0);
