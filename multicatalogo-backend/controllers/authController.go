...
func Login(c *fiber.Ctx) error {
	...
	// Nota: distinguimos dos perfiles: admin y cliente.
	switch {
	case req.Email == "admin@upse.edu.ec" && req.Password == "123456":
		// Si es el administrador, devolvemos token, correo y rol admin.
	...
	case req.Email == "cliente@upse.edu.ec" && req.Password == "123456":
		// Si es un cliente, devolvemos token, correo y rol cliente.
